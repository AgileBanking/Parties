		...
		<div id="list-status" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="title" title="${message(code: 'status.title.label', default: 'Title')}" />
					
						<g:sortableColumn property="titleInt" title="${message(code: 'status.titleInt.label', default: 'Title Int')}" />
					
						<g:sortableColumn property="notes" title="${message(code: 'status.notes.label', default: 'Notes')}" />
					
						<g:sortableColumn property="recStatus" title="${message(code: 'status.recStatus.label', default: 'Rec Status')}" />
				...
				<g:each in="${statusInstanceList}" status="i" var="statusInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${statusInstance.id}">${fieldValue(bean: statusInstance, field: "title")}</g:link></td>
					
						<td>${fieldValue(bean: statusInstance, field: "titleInt")}</td>
					
						<td>${fieldValue(bean: statusInstance, field: "notes")}</td>
					
						<td>${fieldValue(bean: statusInstance, field: "recStatus")}</td>
